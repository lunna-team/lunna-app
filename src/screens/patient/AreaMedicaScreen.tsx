...
import Svg, { Path, Polyline, Line, Rect, Ellipse } from 'react-native-svg';
...
import type { Medication, LabTest, Ultrasound } from '../../types';
...
export function AreaMedicaScreen() {
  ...
  const completedTests = labTests.filter((t) => t.status === 'completed' || t.status === 'abnormal');
          ...
          {tab === 'meds' && (
            <>
              ...
              {medications.length === 0 ? (
                <Text style={styles.empty}>Nenhum medicamento ativo.</Text>
              ) : (
                <View style={styles.checklistCard}>
                  <Text style={styles.checklistHeader}>{today}</Text>
                  {medications.map((m, i) => {
                    const done = doneIds.has(m.id);
                    return (
                      <TouchableOpacity
                        key={m.id}
                        style={[styles.medItem, i === medications.length - 1 && { borderBottomWidth: 0 }]}
                        onPress={() => toggleDone(m.id)}
                        activeOpacity={0.8}
                      >
                        <View style={styles.medInfo}>
                          <Text style={[styles.medName, done && styles.medNameDone]}>{m.name}</Text>
                          <Text style={[styles.medTime, done && styles.medTimeDone]}>
          ...
          {tab === 'exames' && (
            <>
              {ultrasounds.length > 0 && (
                <>
                  <Text style={styles.sectionTitle}>Ultrassonografias</Text>
                  {ultrasounds.map((u) => (
                    <View key={u.id} style={styles.examCard}>
                      <View style={[styles.examIconWrap, styles.iconGreen]}>
                        <WaveIcon color={colors.primary} />
                      </View>
                      <View style={styles.examInfo}>
                        <Text style={styles.examName}>{u.type} · IG {u.ig_weeks}sem</Text>
                        <Text style={[styles.examStatus, { color: colors.primary }]}>
                          {formatDate(u.date)}
                        </Text>
                      </View>
                    </View>
                  ...
                  {completedTests.map((e) => (
                    <View key={e.id} style={styles.examCard}>
                      ...
                      <View style={styles.examInfo}>
                        ...
                        <Text style={[styles.examStatus, { color: e.status === 'abnormal' ? colors.accent : colors.primary }]}>
                          {LAB_STATUS_LABELS[e.status]}
                        </Text>
                      </View>
                      {e.file_url && (
                        <TouchableOpacity
                ...
                <>
                  ...
                  {pendingTests.map((e) => (
                    <View key={e.id} style={styles.examCard}>
                      ...
                      <View style={styles.examInfo}>
                        <Text style={styles.examName}>{e.name}</Text>
                        <Text style={[styles.examStatus, { color: colors.textInactive }]}>
                          {LAB_STATUS_LABELS.pending}
                        </Text>
                      </View>
                    </View>
                  ))}
              ...
              {ultrasounds.length === 0 && labTests.length === 0 && (
                <Text style={styles.empty}>Nenhum exame encontrado.</Text>
              )}
            </>
          )}
        </ScrollView>
...
const styles = StyleSheet.create({
  ...
  empty: { fontSize: 13, color: colors.textMid, textAlign: 'center', marginTop: 20 },
